...
export const QUALITY_RATINGS = {
  AGAIN: 0,        // Complete blackout - "Ask Me Again"
 ...
 */
const calculateEasinessFactor = (currentEF: number, quality: QualityRating): number => {
 ...
 */
const calculateInterval = (repetitions: number, easinessFactor: number): number => {
  switch (repetitions) {
    case 0:
    ...
    default:
      ...
      return Math.round(calculateInterval(repetitions - 1, easinessFactor) * easinessFactor);
  }
};
...
export const calculateSM2 = (
  ...
  const newEasinessFactor = calculateEasinessFactor(currentParams.easinessFactor, quality);
 ...
 */
export const isCardDue = (nextReviewDate: Date, currentDate: Date = new Date()): boolean => {
...
export const getDueCards = <T extends { sm2: SM2Parameters }>(
  ...
  return cards.filter(card => isCardDue(card.sm2.nextReviewDate, currentDate));
};
...
export const calculateReviewStats = <T extends { sm2: SM2Parameters }>(
  ...
  return {
    ...
    totalReviews,
    averageQuality: totalQualityCount > 0 ? totalQualitySum / totalQualityCount : 0
  };
};

/**
 * Utility function to format review statistics for display
 */
export const formatReviewStats = (stats: ReviewStats): Record<string, string> => {
  const percentage = (value: number, total: number) => 
    total > 0 ? `${Math.round((value / total) * 100)}%` : '0%';
  
  return {
    totalCards: stats.totalCards.toString(),
    dueCards: stats.dueCards.toString(),
    overdueCards: stats.overdueCards.toString(),
    masteredCards: `${stats.masteredCards} (${percentage(stats.masteredCards, stats.totalCards)})`,
    difficultCards: `${stats.difficultCards} (${percentage(stats.difficultCards, stats.totalCards)})`,
    averageEasinessFactor: stats.averageEasinessFactor.toFixed(2),
    totalReviews: stats.totalReviews.toString(),
    averageQuality: stats.averageQuality.toFixed(1)
  };
};
