...
import { firestore, isFirebaseError, getFirebaseErrorMessage } from './firebase';
...
export const createUserProfile = async (userId: string, profileData: any): Promise<FirestoreResult> => {
  try {
...
export const saveFlashcard = async (cardData: any): Promise<FirestoreResult> => {
  try {
    const currentUser = getCurrentUser();
    if (!currentUser) {
      return { success: false, error: 'User must be authenticated to save cards.' };
    }
...
export const saveFlashcardsBatch = async (cards: any[]): Promise<FirestoreResult> => {
  try {
    const currentUser = getCurrentUser();
    if (!currentUser) {
      return { success: false, error: 'User must be authenticated to save cards.' };
...
export const getUserFlashcards = async (): Promise<FirestoreResult<any[]>> => {
  try {
    const currentUser = getCurrentUser();
    if (!currentUser) {
      return { success: false, error: 'User must be authenticated to get cards.' };
    }
    
    const cardsCollection = getUserCardsCollection(currentUser.uid);
    const cardsQuery = query(cardsCollection, orderBy('createdAt', 'desc'));
    
    const querySnapshot: QuerySnapshot<DocumentData> = await getDocs(cardsQuery);
...
export const getDueFlashcards = async (): Promise<FirestoreResult<any[]>> => {
  try {
    ...
    const querySnapshot: QuerySnapshot<DocumentData> = await getDocs(dueCardsQuery);
...
export const updateFlashcardProgress = async (
  ...
  try {
    const currentUser = getCurrentUser();
    if (!currentUser) {
      return { success: false, error: 'User must be authenticated to update progress.' };
    }
...
// Delete a flashcard
export const deleteFlashcard = async (cardId: string): Promise<FirestoreResult> => {
  try {
    const currentUser = getCurrentUser();
    if (!currentUser) {
      return { success: false, error: 'User must be authenticated to delete cards.' };
    }
...
export const migrateGuestDataToUser = async (guestData: any): Promise<FirestoreResult> => {
  try {
    const currentUser = getCurrentUser();
    if (!currentUser) {
    ...
    if (migrationResult.success) {
      console.log(`Successfully migrated ${guestData.cards.length} cards from guest mode`);
...
export const subscribeToUserFlashcards = (callback: (cards: any[]) => void): (() => void) => {
  const currentUser = getCurrentUser();
  if (!currentUser) {
